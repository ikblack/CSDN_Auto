...
namespace Flexlive.CQP.CSharpPlugins.Demo
{
    ...
    public class MyPlugin : CQAppAbstract
    {
        ...
        public override void GroupMessage(int subType, int sendTime, long fromGroup, long fromQQ, string fromAnonymous, string msg, int font)
        {
            // 处理群消息。
            /*
            var groupMember = CQX.GetGroupMemberInfo(fromGroup, fromQQ);

            CQX.SendGroupMessage(fromGroup, String.Format("[{4}]{0} 你的群名片：{1}， 入群时间：{2}， 最后发言：{3}。", CQX.CQCode_At(fromQQ),
        ...
        {
            // 处理群文件上传事件。
            // CQX.SendGroupMessage(fromGroup, String.Format("[{0}]{1}你上传了一个文件：{2}", CQX.ProxyType, CQX.CQCode_At(fromQQ), file));
        }
        ...
        {
            // 处理群事件-管理员变动。
            // CQX.SendGroupMessage(fromGroup, String.Format("[{0}]{2}({1})被{3}管理员权限。", CQX.ProxyType, beingOperateQQ, CQE.GetQQName(beingOperateQQ), subType == 1 ? "取消了" : "设置为"));
        }
        ...
        {
            // 处理群事件-群成员减少。
            // CQX.SendGroupMessage(fromGroup, String.Format("[{0}]群员{2}({1}){3}", CQX.ProxyType, beingOperateQQ, CQE.GetQQName(beingOperateQQ), subType == 1 ? "退群。" : String.Format("被{0}({1})踢除。", CQE.GetQQName(fromQQ), fromQQ)));
        }
        ...
        {
            // 处理群事件-群成员增加。
            // CQX.SendGroupMessage(fromGroup, String.Format("[{0}]群里来了新人{2}({1})，管理员{3}({4}){5}", CQX.ProxyType, beingOperateQQ, CQE.GetQQName(beingOperateQQ), CQE.GetQQName(fromQQ), fromQQ, subType == 1 ? "同意。" : "邀请。"));
        }
        ...
        {
            // 处理请求-好友添加。
            // CQX.SetFriendAddRequest(responseFlag, CQReactType.Allow, "新来的朋友");
        }
        ...
        {
            // 处理请求-群添加。
            // CQX.SetGroupAddRequest(responseFlag, CQRequestType.GroupAdd, CQReactType.Allow, "新群友");
        }
    }
}
